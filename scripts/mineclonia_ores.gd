			...
			for cz in range(first.y,last.y+1):
				for cx in range(first.x,last.x+1):
					...
					for cluster in count:
						var origin := Vector3i(rng.randi_range(cx*80+CHUNK_OFFSET,cx*80+CHUNK_OFFSET+80-rule.size),rng.randi_range(min_y,max_y-rule.size+1),rng.randi_range(cz*80+CHUNK_OFFSET,cz*80+CHUNK_OFFSET+80-rule.size))
						var seed_value: int = rng.randi()
						if origin.x > hi.x or origin.x+rule.size <= lo.x or origin.z > hi.y or origin.z+rule.size <= lo.y: continue
