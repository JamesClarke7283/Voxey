...
var failed: int = 0
func _init() -> void:
	OS.set_environment("VOXEY_DATA_DIR","/tmp/voxey-alchemy-check-worlds")
	call_deferred("run")
...
func run() -> void:
	...
	game.pause()
	await load("res://tests/alchemy_checks.gd").run(self,game)
	await load("res://tests/pouch_checks.gd").run(self,game)
	print("ALCHEMY TESTS: %d passed, %d failed"%[passed,failed])
	game.queue_free()
