...
func run() -> void:
	...
	sun.light_color = Color("fff0cd"); sun.light_energy = 0.55
	...
	camera.current = true
	var kinds: Array = ["cow","sheep","pig","chicken","zombie","skeleton","spider","creeper"]
	for i in 8:
		var mob := Creature.new()
		mob.kind = kinds[i]; mob.set_physics_process(false)
		mob.position = Vector3(-3.6+(i%4)*2.4,2.0 if i < 4 else -0.7,0)
	...
	label_at(ui,"Jointed animals · Original pixel skins · Clearer faces and silhouettes",Vector2(55,109),16,Color("a5b3a5"))
	for i in 4:
		label_at(ui,["COW","SHEEP","PIG","CHICKEN"][i],Vector2(245+i*250,425),16,Color("dfd7bc"))
		label_at(ui,["ZOMBIE","SKELETON","SPIDER","CREEPER"][i],Vector2(235+i*250,725),16,Color("dfd7bc"))
	for i in 5: await process_frame
	await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute("/tmp/voxey-shots")
	root.get_texture().get_image().save_png("/tmp/voxey-shots/10_creature_gallery.png")
	...
	await RenderingServer.frame_post_draw
	root.get_texture().get_image().save_png("/tmp/voxey-shots/11_creature_walk.png")
