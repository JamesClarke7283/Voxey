...
Voxey gives 4–6 player XP per trade and five additional XP on level-up. Work restores stock twice per day, with over 120 active seconds before a second restock and a 600-second fallback to a new working day. Demand is adjusted by `2 * uses - stock`. Reputation is stored per local player profile, with trading discounts and penalties for hitting villagers. Voxey also has a three-wave pillager raid and Hero of the Village discounts; multiplayer gossip and zombie curing are not implemented. Enchanted offers draw from the full enchantment catalog, respecting equipment compatibility and source maximum levels.
