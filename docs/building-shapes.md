...
The first 60 material families provide 120 craftable building items: six classic woods, stone and cobble variants, bricks, sandstone, deepslate, tuff, Nether and End masonry, quartz, granite/diorite/andesite, blackstone, iron/gold, all sixteen terracottas, and the four cut copper oxidation stages.
...
- The recipe grid shows shape previews. Stonecutters use the source's eligible materials and yield two slabs or one stair per block, including raw-stone routes to finished shapes. Wood and decorative metal shapes remain crafting recipes. Wood slabs have 7.5 seconds of fuel value; stairs have 15.
...
The six-suite regression run covers these shapes alongside the rest of the gameplay checks; the dedicated building suite passes 137 checks including the 5,000 source stair corner cases, tuff and masonry comparisons and native noise comparisons. Script diagnostics are treated as test failures even when Godot returns exit status zero.

This is partial building parity. [Chiseled, cracked and mossy stone bricks](ruined-portals-source.md) are now present, with cracked bricks smelted from the plain ones. Several base materials and their shapes are still missing, including additional woods, smooth/red sandstone, concrete, prismarine and further polished masonry variants. Some source slabs also have distinct side textures. Other attachment types and exact material mining/light behavior remain part of the wider parity work.
